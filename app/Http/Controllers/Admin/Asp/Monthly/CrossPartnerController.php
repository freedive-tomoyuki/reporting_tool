...
class CrossPartnerController extends MonthlyCrawlerController
{
    ...
    public function crosspartner( $product_base_id ) //OK
    {
        ...
        $products =  json_decode($this->monthlySearchService->BasetoProduct( 10, $product_base_id ),true);
                ...
                try{
                        ...
                        foreach ( $product_infos as $product_info ) {
                            ...
                            for ( $i = 0; $i < 2; $i++ ) {
                                ...
                                if(trim( preg_replace( '/[\n\r\t ]+/', ' ', str_replace( "\xc2\xa0", " ", $crawler_for_site->filter( 'table.highlight > tbody > tr:nth-child('.$iPlus.') > td:nth-child(1)' )->count()) ) )  >  0 ){ 
                                     //throw new \Exception('サイト情報を取得できませんした。'); };
